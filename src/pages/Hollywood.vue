<template>
  <Layout>
    <div>hej {{ powitanie }}</div>
    <button @click="counterPlus">KLIK</button>
    <p>
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Eveniet
      repellendus dolorem eligendi aspernatur voluptate tempore pariatur earum
      eos ipsa, accusamus quas animi eius nisi libero, repudiandae blanditiis
      delectus praesentium tenetur. Lorem ipsum dolor sit, amet consectetur
      adipisicing elit. Quisquam praesentium totam pariatur rem ad labore minus,
      incidunt numquam, sapiente quis aliquam recusandae quas, odio ut
      perspiciatis dolores mollitia! Sequi, blanditiis. lorem300 Lorem ipsum
      dolor sit amet consectetur adipisicing elit. Fugiat sit praesentium error,
      consectetur nemo earum et. Culpa, impedit. Porro, praesentium assumenda!
      Pariatur dignissimos voluptatum nemo distinctio autem dolorum aut ab.
      Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda
      repudiandae suscipit deserunt rem ducimus inventore, sunt ex ratione
      mollitia! Omnis voluptas error sed laborum rerum eligendi, corporis
      quibusdam dolorem nesciunt!
    </p>
  </Layout>
</template>

<script>
export default {
  data() {
    return {
      powitanie: "hej",
    };
  },
  methods: {
    counterPlus: () => console.log("hej"),
  },
};
</script>

<style></style>
